...
{
  home.file.".config/hypr/scripts/WaybarCava.sh" = {
    executable = true;
    text = builtins.replaceStrings ["__DOLLAR__"] ["$"] '' 
      ...
      bar="▁▂▃▄▅▆▇█"
      dict="s/;//g"
      bar_length=__DOLLAR__{#bar}
      for ((i = 0; i < bar_length; i++)); do
        dict+=";s/$i/__DOLLAR__{bar:$i:1}/g"
      done
      
      # Single-instance guard (only kill our previous instance if it’s still alive)
      RUNTIME_DIR="__DOLLAR__{XDG_RUNTIME_DIR:-/tmp}"
